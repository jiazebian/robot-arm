...
class Calibration(object):
    def __init__(self, width: int = 640, height: int = 480, size: tuple = (9, 6), size_step: float = 1.0,
        ...
        self.w = width
        self.h = height
        self.size = size  # 棋盘格内角点数量，例如 (9, 6) 表示 9x6 个内角点
        self.size_step = size_step  # 棋盘格每个方格的边长，默认 1.0（单位：米）
        self.img_num = img_num  # 用于标定的图像数量
        self.cal_path = cal_path  # 数据存储路径

        self.camera_Intrinsics = None  # 内参矩阵
        self.distCoeffs = None  # 畸变系数
        self.rvecs = None  # 旋转向量
        self.tvecs = None  # 平移向量
        self.RT_cam2gripper = None           # core: AX=XB，机械臂末端的齐次变换矩阵，相机坐标系与机械臂末端坐标系之间的空间关系
    ...
    @staticmethod
    def Cartesian2Homogeneous(Ola_pose):  # 位姿转齐次矩阵
    ...
    def chessboard(self, index, image):  # 检测棋盘格角点，为相机标定提供角点数据
        """
        image: 原始图像
        ...
        _, points = cv.findChessboardCorners(gray, board_size)                 # 检测角点，points是二维坐标系
        try:
        ...
        # 生成棋盘格内角点的三维坐标(cm)
        # 创建一个形状为 (N, 3) 的零矩阵，其中 N = self.size[0] * self.size[1]，表示棋盘格内角点的总数
        obj_points = np.zeros((self.size[0]*self.size[1], 3), np.float32)
        # 生成棋盘格内角点在 x-y 平面上的二维坐标（计算方法是格子数量*边长），并赋值给 obj_points 的前两列，z 坐标保持为 0
        # T.reshape(-1, 2)是为了将网格展平成 (N, 2) 的数组，每行是一个点的 (x, y) 坐标
    ...
    def calibrate(self, all_obj_points: list=None, all_points: list=None, default=False):
        """
        all_obj_points:世界三维坐标系 序列，上面的函数算出来的

        ：二维像素角点坐标系 序列

        计算相机内参矩阵和畸变系数，为后续畸变校正和位姿计算提供相机参数
        """
        if all_obj_points is not None and all_points is not None:  # 确保输入参数有用
            pass
        else:
            all_obj_points = self.all_real_points
            all_points = self.all_pix_points

        assert len(all_obj_points) == len(all_points) == self.img_num  # 防止数据不匹配导致标定失败

        # 核心处理，计算内参矩阵 (self.camera_Intrinsics)：描述相机的焦距和主点位置。
        # 畸变系数 (self.distCoeffs)：描述镜头的畸变（径向和切向）。
        # 旋转向量 (self.rvecs)：每张图像的棋盘格相对于相机的旋转。
        # 平移向量(self.tvecs)：每张图像的棋盘格相对于相机的平移
